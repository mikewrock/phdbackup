...
BCAP_HRESULT bCap_ControllerConnect(	int iSockFd,
	...
	if (pPacket != NULL){
...

		{
			...
			printf("precpacket success %d\n", pRecPacket);
			if (pRecPacket != NULL){
			
				printf("bcap success2\n");
				hr = bCapSendAndRec(iSockFd, pPacket, pRecPacket);
					printf("bcap success3\n");
				if SUCCEEDED(hr){
					copyValue(plhController, pRecPacket->pArg->data, 4);
						printf("bcap success4\n");
				}
...
BCAP_HRESULT bCap_RobotExecuteSlaveMove(int iSockFd, u_long lhRobot, char *pStrCommand, float *pfOption, void *pResult){
	...
	if (pSndPacket != NULL){
		...
		{	/* R4\94z\97\F1\82ňʒu\82\F0\8Ew\92\E8 */
			pArg = Arg_Create( VT_R4 | VT_ARRAY, 8, sizeof(float) * 8, pfOption);	/* 8\8E\B2\82Ԃ\F1 */
			if (pArg != NULL){
				Packet_AddArg(pSndPacket, pArg);
			}
		}
...
pbSendData = bMalloc(255);
printf("malloc sending packet\n");
	if (pbSendData != NULL){
...
printf("Packet_Serialize\n");
					for (j= 0; j < pArgPtr->lArrays; j++){
						...
						switch (iVarType){
							...
							case VT_BSTR:
								{
								printf("Packet_Serialize3\n");
									u_long lStrLen =0;
									copyValue( &lStrLen, pbValue, BCAP_SIZE_ARGSTRLEN);
									printf("Packet_Serialize9 %lu\n",lStrLen);
...
printf("Packet_Serialize10 %d : %d : %lu\n", pDstPtr, pbValue, lStrLen);
									pDstPtr += copyValue( pDstPtr, pbValue, lStrLen);	/* Set string data */
									printf("Packet_Serialize11\n");
									pbValue += lStrLen;
								}
								break;
...
#else
printf("lendian %d %d %d\n", pDst, pVntValue, lLength);
