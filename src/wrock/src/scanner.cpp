...
void scanCallback (const sensor_msgs::LaserScan::ConstPtr& scan_in)
{
  ...
  projector.transformLaserScanToPointCloud("/base_link",*scan_in,
          cloud,listener);
...
int main(int argc, char** argv)
{
  ros::init(argc, argv, "cube_move");
    ...
    ros::NodeHandle n;
ros::Subscriber joint_cmd_sub = n.subscribe("/scan", 1000, scanCallback);
...
ROS_INFO("Zeroing Modules");
ret = PCube_moveRamp(dev, modJ1, 0 , 1.5, 2);
ret = PCube_moveRamp(dev, modJ2, 0 , 1.5, 2);
ret = PCube_moveRamp(dev, modJ3, 0 , 1.5, 2);
...
//Initialize TF information
    ...
    odom_trans.child_frame_id = "axis";
        ...
        joint_state.name[8] ="cube_5_joint";
